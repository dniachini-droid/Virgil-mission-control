...
**0.10.11 Standards unchanged, plus.** §6 stands. CI now runs on every push (`.github/workflows/checks.yml`; `pnpm check` includes `verify:owner`; `test/required-checks.test.ts` holds the wiring) and nothing in it is weakened. The artifact is named with the viewing point first (`v8-s2-…`). The size is reported honestly; the cast, the floor, the lighting and the nebula are not degraded to chase a number.
