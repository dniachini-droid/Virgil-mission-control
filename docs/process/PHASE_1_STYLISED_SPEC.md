...
**Superseded in part by the V8 direction of 2026-09-08 (§0.10 below).** The owner reviewed V7 and gave eight changes, then three amendments during the V8 pass; §0.10 records them verbatim and says which of §0.1–§0.9 and §2–§4 they override. The V7 sections stand as the record of what V7 was asked to be.
...
### 0.10 V8 — the owner's review of V7, and what it supersedes

Relayed by the owner console on 2026-09-08 from the owner's review of V7, with three amendments relayed during the V8 pass. The owner's words are quoted verbatim; the console's and the build session's readings are marked as such. Written down **before** the V8 work began, for the reason §0 gives.

**0.10.1 The layout is symmetrical.** *"each console should be behind the main virgil console, one directly behinmd it and the other to the left and right, so its symmetrical."* Supersedes §0.4's depth arrangement and `layout.stations`. Virgil's console stays where it is, in front and central; the three role consoles stand behind it — one directly behind, one to its left, one to its right — symmetrical about the centre line. Which role takes which slot the owner did not say; the build session puts the Fabricator on the left, the Prover directly behind, the Keeper on the right, in the order of the three hops, and records that as its own choice. The portrait-framing problem §0.4 solved with depth is now solved differently (0.10.4), and the build session verifies at 390 × 664 that all four still fit and says what it finds.

**0.10.2 The consoles' own screens carry the information; the floating panels go.** *"each screen of the actual consoles (created by meshy) should have the information on it."* This is §0.9 built, and it retires the three floating per-agent panels — which is what answers *"too many screens"*, raised again on V7. Virgil's three slabs stay, as §0.9 records. The treatment is the one the owner specified in §0.9: *"make them compleetyley black, reflective, and text sitting slightly under it"* — the same `glass.ts` material, one implementation, not a second. **What the models turned out to carry, measured 2026-09-08 on the shipped payloads:** each station has exactly one usable screen — a recessed navy panel in a cream bezel at the top, tilted back 12–14°, facing the front — with pale writing baked into it; the other dark surfaces are desk tops, side panels and the backs. The screens are **navy, not black**, the same navy as the trim, so the visor's paint rule cannot find them from the texture (§0.9 hoped it might; it does not), and the baked writing means a per-pixel paint mask would punch holes through the content. So `asset-pipeline/fit-screen.mjs` selects the screen by **geometry** — a measured region, the facing, and the darkness of most samples — and the whole selected triangle is the screen, the shader's rule set to keep every pixel; the mask records that rule. Same face, same glass, same builder.

**0.10.3 The circular window goes.** *"remove the circular window. I asked for that."* Supersedes §0.1's reading that the arch could stay as a skyline element, and §2's "the porthole becomes a free-standing arch". The arch is removed from the tabletop. The model stays in `assets/models/candidates/` as preserved evidence, and its payload stays in the bundle because the retired room's window frame still uses it (the room is retired, not removed, §0.1); dropping it from the bundle is one decision away and would save about 0.9 MB.

**0.10.4 The camera is almost level.** *"The camera starts too high up so everything looks squished. It should start almost levelm with the characters, maybe a tiny bit higher."* Supersedes §2's 30° and §0.4's 38° portrait camera. A few degrees above the characters' eye height, not thirty; the orbit stays constrained to a wide arc. A consequence the build session records rather than hides: with a near-level camera, whoever stands directly behind Virgil is partly behind him from the default view, and a symmetrical set is wide where a phone is tall, so at 390 × 664 the cast is a band across the middle of a tall frame. The screenshots say what that looks like.

**0.10.5 The planet and the station move up.** *"in that position, the planet and space station in the background is covered by the floating tabletop, so they need to be moved up."* The owner worked this consequence out themselves. The nebula stays as the surrounding backdrop with the procedural stars and grain over it.

**0.10.6 The screen animations: craft, not content — and the receiving beat is the centrepiece.** *"The 'awaiting' and 'reviewing' animations I feel can be made to be more..... detailed. they look a little basic. The windows dont have to display more information but it would be nice to have the animations look more advanced and not something that is basic clip art that is animated."* Then, amending: *"yes, and make the animations more... futeristic. put work into those animations, make it look detailed. And don't make the 'recieving' screen so quick, it should really have an awesome animation of information being transmitted and recieving."* And: *"when virgil recieves a pass or a block - the information should come back onto the screen and almost converge, or like millions of pieces of data coming to gether, or some other creative way."* **Supersedes §4's "Motion stepped and snappy — hard jumps between states, because smooth easing reads realistic."** The owner has now said three times that the motion reads as animated clip art; the rule is reversed: non-linear easing, weight and settle rather than snap, elements arriving in sequence, secondary motion trailing primary by a beat, depth through layering against the glass, a suggestion of process rather than a loop. Not more words, not more readouts — §4's subtraction rule stands. The receiving beat gets real duration and distinct stages (something arriving, being received, unpacked, settling into a state); the returning beat is built as its **inverse** — receiving disperses, returning converges, many small pieces gathering into one verdict — so the two read as a cycle. The demonstration's timeline is lengthened to give both beats their time.

**0.10.7 Counters, real vocabulary, four verdicts, and the evidence beneath a verdict.** *"there should also be a counter for the keeper, for example how many issues it found and blockers it found, no? Like aside from just doing the work, it should have a count, same as the proover, like the outcomes of its tests as it works (but it shoudlnt recquire a change in how the actual agent reacts)."* The Keeper's screen counts findings as they are raised and how many are blocking (`constitution/REVIEW_POLICY.md`: every finding has a severity; `packages/agent-contracts` `Severity`: `blocking`, `major`, `minor`, `informational`); the Prover's counts check outcomes as they run (`constitution/authority.json` `checkResults`: `running`, `passed`, `failed`, `skipped`). The numbers accumulate as the work runs and resolve at the verdict; they stay illustrative and the `ILLUSTRATIVE · NOT REAL STATE` band stays baked in. The agents' faces, lights and behaviour do not change. The verdict moment is made an event, not a colour change. **Then, as a correctness fix:** the console observed that two colours for four verdicts is inaccurate, and the owner confirmed — *"yes, the screen should show the 4 verdicts."* `constitution/REVIEW_POLICY.md` and `STATE_LANGUAGE.md` name exactly four: `PASS`, `PASS_WITH_NON_BLOCKING_FINDINGS`, `BLOCKED`, `INSUFFICIENT_EVIDENCE`; the two collapsed were the ones that carry the most meaning (every real review here has returned `PASS_WITH_NON_BLOCKING_FINDINGS`, never a bare `PASS`; `INSUFFICIENT_EVIDENCE` is not a failure and must not be red). Each gets a distinct visual state — in geometry, not colour alone, as the policy requires — and the demonstration's third loop now ends in `INSUFFICIENT_EVIDENCE` so all four are seen. A consequence for the demonstration's honesty: the Keeper's illustrative review raises findings, none blocking, so its verdict is `PASS_WITH_NON_BLOCKING_FINDINGS`, not `PASS`. **And the evidence beneath a verdict** (candidate 1 of 0.10.10, approved): a verdict alone is a claim; the deterministic checks that produced it are the proof — `CLAUDE.md`'s "a builder's success report is not evidence" drawn. A small number of counts whose *kinds* are the ones this repository really produces (tests, typecheck tasks, requests and off-document requests, tethers — the shapes of `CheckRun` and the verification record in `packages/agent-contracts`), the numbers illustrative and labelled, never crowding the verdict.

**0.10.8 Turning, not walking — no rig needed.** *"In terms of the walking, im not sure how we'd do that yet. Just postion the consoles like I said, and have each agent at the console, sightly to the left so it doesnt obstruct the screens, faciung forward. When they get sent work, an animation plays on their face/eyes and they turn aroundfacing the screeen. When its done, they turn aroiund and face the front again."* Supersedes §0.7's idle-aside position and the glide. Each agent stands at their console, offset to its left, facing the camera; on work arriving, a face animation, then a whole-body turn to face their screen; on completion, the turn back. One axis, eased in and out, a slight overshoot and a settle, the breathing continuing through it. **The three walking rigs the owner has pending are not needed for this**, and nothing waits on them.

**0.10.9 A seam for tubes — recorded, not built.** The owner: *"Maybe later we can have tubes connecting all of the consoles and as part of the handoff to another agent, light travels across the tube to the correct console, and back again."* Recorded here as owner direction for a later pass, with the console's reasoning: a tube carrying light from console to console makes the hand-off one continuous event rather than a gesture here and a screen change there, shows which station received the work, in which direction, and when the result came **back**; and it gives the receiving animation its duration honestly, because the transfer takes as long as the light takes to travel. The symmetrical layout of 0.10.1 is what makes it geometrically natural — three tubes from one console to three. What V8 builds toward it: every receiving and returning animation has a **defined arrival point** — a place on the screen and a moment — expressed in one place (`screens/arrival.ts`) so a tube terminating at a console can feed into it without the animation being rebuilt. The build record names it.

**0.10.10 Four candidates offered to the owner, two selected.** Asked *"pleease conisder any other inforamation that show whats actually happening that we can do"*, the console offered four, all to stay illustrative and labelled while nothing real drives them. The owner: *"i like 1 and 3 and yes, the screen should show the 4 verdicts."*

1. **The evidence beneath a verdict** — a verdict with the deterministic checks that produced it is a proof, not a claim; the strongest of the four. **Selected; built in V8** (0.10.7).
2. **The repair budget** — `constitution/REPAIR_LIMITS.md` allows one bounded repair and one fresh re-review per lineage, `maxCyclesWithOwner = 2`; a counter showing that budget being consumed tells the owner the system is running out of automatic recourse before it asks for them — predictive rather than reactive. **Presented to the owner and not selected.** Kept here so a future reader sees what was offered and declined.
3. **What is waiting on the owner** — the only thing on screen they must act on, and the only thing rendered that way. The gate is a real concept (`constitution/AUTHORITY_TIERS.md`, OD-0006; merge is owner-only in every phase), and the demonstration had no owner-gate moment at all, so this is a new beat, not a readout: the system stopping and turning to them — every agent idle, every screen quiet, one thing lit. **Selected, at a priority below the receiving and returning animations**; built if it fits, deferred and said so if it threatens the pass.
4. **The frozen candidate SHA** — review is of one exact immutable commit; showing that identity not changing while judgement proceeds is itself the statement. **Presented to the owner and not selected.** Kept for the same reason as 2.

**0.10.11 The consoles are spotlit while they work, and the light fades slowly.** *"can we please ensure that all of the consoles remain normal when not in use, but when they are doing work, they are lit up lighter, almnost as if a spotlight is on them, and when they are done, the spotlitght slowly fades down."* Three states per console, Virgil's included: normal when idle, lifted as though spotlit while working, fading slowly when done. Not a shadow-casting light — this has to run on a phone — but an emissive lift on the console's own material following its own texture, and a soft pool of light on the disc beneath it, which is what sells it as a spotlight (`room/Models.tsx`, `spotLevel`). The console's reading of the fade, which the build session adopts: fast rise, slow decay, the decay long enough to still be visible while the next console is starting, so a console still glowing faintly says *this one worked recently* and the disc holds a short history of itself; an unlit console reads as nothing running, which is the honest state and needs no label.

**0.10.12 The agents' screens are dark when idle; a screen turns on and off like a television.** *"we should avoid having every screen on if its not in use. Ie - when the keepewr isnt doing work it should stay off his screen. And when it turns on, make it turn on like a tv, and whene it turns off, an exadurated old tv turning off type animation, where it shrinks in circular mannner - you know how old tvs. used to turn off."* Power follows the station's work. Power-on is a CRT warming up — a moment of nothing, a bloom of light and a bright line, the picture opening, the brightness overshooting and flickering into stability; not a fade-in. Power-off is the exaggerated collapse — the picture collapses vertically into a thin bright line, the line contracts to a point, an afterglow dot lingers and fades (`screens/crt.ts`). It is done as uniforms on the display shader, never by scaling geometry: the glass, the case and the bezel do not move, only the image inside them, which is what makes it read as a screen switching off rather than an object shrinking. The classic collapse was built; if *"shrinks in circular mannner"* meant an iris closing inward, that is one function to change, and the owner document says which was built. **Virgil's three slabs stay on** — the console's recommendation, built unless the owner says otherwise: he is always conducting; the three wake when summoned and go dark when done, which is true of how the system works. This also answers *"too many screens"* a second way: at any moment most of them are dark.

**The canonical light sequence**, recorded so the next pass adds the tube to the front of it and nothing has to be re-authored: light travels the tube *(later)* → the receiving console's spotlight lifts → its screen powers on like a CRT → the work animation runs → the verdict converges back → the screen collapses off → the spotlight decays slowly. One continuous light story, authored as one, rather than three effects that happen near each other; light now doing the work that text was doing badly.

**0.10.13 A coherence defect the owner found by reading closely.** *"when the fabricator is building, virgil's screen says awaiting review. its actually awaiting on the build."* A truthfulness defect, not a wording preference, and fixed at the cause rather than the string: Virgil's slabs no longer carry a fixed label across beats the sequence moves through; they are **driven from the candidate's state in the constitution's own words** — `constitution/STATE_LANGUAGE.md`'s candidate states and the transition table in `constitution/authority.json`, which `packages/domain` encodes — so each beat names the state the sequence is genuinely in (`BUILDING` while the Fabricator builds, `BUILDER_REPORTED_COMPLETE` at his claim, `VERIFICATION_INCOMPLETE` while the Prover runs, `READY_FOR_REVIEW` at his pass, `REVIEW_IN_PROGRESS` while the Keeper reviews, `PASS_WITH_NON_BLOCKING_FINDINGS` at his verdict, `SAFE_TO_MERGE` at the owner gate; `BLOCKED` and `INSUFFICIENT_EVIDENCE` in the other loops), and `test/demo.test.ts` holds every word to the vocabulary and every step to an allowed transition. Two beats have no word in the state language and are recorded rather than invented for: the rest between loops, when no lineage exists, and the hand-off itself, the grant issued before the agent starts, which the vocabulary folds into the state the candidate is already in. The audit of the rest of the loop found two more of the same class, both fixed: the candidate slab's *phase words* (`HAND-OFF`, `BUILD`, `CLAIMED`, `VERIFY`, `VERDICT`, `REVIEW`, `REVIEWED`) were invented phrasing and are replaced by the candidate states; and its identity-shaped hex string walked every 0.7 s while judgement proceeded, which pictured a candidate changing under review — it is now fixed for the candidate. What belongs in the record: the honesty band was true, and the label does not excuse the content being incoherent. An illustrative demonstration must still depict a sequence that could happen; otherwise the band guards against one falsehood ("this is real") while permitting another ("this is how it works"). That is the ground the Interface Keeper role exists to cover, and the owner caught it before any reviewer did.

**0.10.14 Standards unchanged, plus.** §6 stands. CI now runs on every push (`.github/workflows/checks.yml`; `pnpm check` includes `verify:owner`; `test/required-checks.test.ts` holds the wiring) and nothing in it is weakened. The artifact is named with the viewing point first (`v8-s2-…`). The size is reported honestly; the cast, the floor, the lighting and the nebula are not degraded to chase a number.
