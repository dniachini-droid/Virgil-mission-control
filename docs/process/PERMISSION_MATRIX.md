...
| `.claude/settings.json` allow and deny rules | Allow: fixed workspace commands and read-only git commands only. Deny: destructive Git commands, pushes to main by branch or refspec, `pnpm exec`/`dlx`, secret file reads, Write and Edit into every path-shaped protected boundary (constitution, master commission, accepted owner decisions, raw sources, gate schemas) | Enforced for Write/Edit/Read/Bash pattern matches; Bash-mediated file writes through an allowed command are not covered; coverage is validated by tests that model the harness matcher against concrete commands (the harness itself is not exercised) |
