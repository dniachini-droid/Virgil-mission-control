...
| `.claude/settings.json` deny rules | Named destructive Git commands, secret file reads, Write/Edit into protected paths | Enforced for Write/Edit/Read/Bash pattern matches; Bash-mediated file writes (heredocs, sed) are not covered |
| Authority grants and gate engine | Permitted paths, SHA currency, reviewer independence, repair count, merge and deploy authority | Computed deterministically from evidence; Phase 0 provides the functions and fixtures, Phase 3 wires them to live sessions |
