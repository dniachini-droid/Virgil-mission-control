...
At rest he holds a standing pose and **breathes**: a rise of up to 1.5 cm, a sway of ±0.6° and a turn of ±1.1°, at three rates that never share a period, so it never visibly loops. The sway is smaller than the 1.5° first considered because it pivots at his feet, and at 1.5° a foot's edge would dip through the floor. The Prover breathes the same way, out of phase with him. Both are still with reduced motion. *[Correction added 2026-09-08 by the V6 pass, on finding KR-55 of the review of `a4f8b70`: this sentence did not disclose that with reduced motion V5 removed both faces and their lights entirely — `Visor.tsx` returned nothing — so Virgil showed his baked grin and the Prover a blank dome, and the blocked-versus-passed distinction that `docs/art-direction/OPERATIONAL_ANIMATION.md` forbids losing was lost. V6 draws a static face instead and tests it; see `PHASE_1_HOW_TO_LOOK_V6.md`. The sentence above is left as V5 wrote it.]*
