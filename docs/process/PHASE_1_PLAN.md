...
Sources: `docs/product/VIRGIL_MASTER_COMMISSION.md` sections 5.5 and 11 and Amendment 1 (including the Phase 1 vertical-slice amendment); `docs/process/PHASE_1_BRIEF.md`; `docs/architecture/ENFORCEMENT_BOUNDARIES.md`; `docs/architecture/PERFORMANCE_STRATEGY.md`; the accepted ADRs `ADR-0003` (rendering stack) and `ADR-0010` (labels as canvas textures); `docs/art-direction/ART_BIBLE.md` and `ROLE_PERFORMANCE_BIBLE.md`; `docs/art-direction/approved/README.md`; `assets/README.md`; `assets/licenses/ASSET_PROVENANCE.md`; owner decisions OD-0002 to OD-0006 (currently at `docs/decisions/proposed/`, treated as accepted authority per the owner's instruction).
...
|---|---|---|---|
| **V0** | Can you open the file at all? (content is the old rejected spike — not an art judgement) | ~1 session | One session, and the delivery mechanism itself: the build target, the inliner and the viewing script, plus a delivery route that needs your separate authorisation |
...
1. **Unblock the base.** `pnpm check` currently fails on `main`. Your own commit `9627bae` ("Update settings.json") removed two lines that a test still requires. That is not a criticism — you may have removed them deliberately so sessions can file decision records — but the test was not updated to match, so the repository's own check suite is red before Phase 1 starts. Part 7, P0 records what you have since changed on `main`, what still stops the check, and what it now needs from you.
...
Two accepted ADRs (authority layer 3) are load-bearing here, and both favour the mechanism. `ADR-0003` fixes the rendering stack to three.js, React Three Fiber, drei and postprocessing — plain JavaScript libraries a bundler can flatten into one classic script. `ADR-0010` already replaced network-fetched SDF text with canvas-texture labels because *"drei's Text (troika) fetches a default font from the network; the sandbox and offline use forbid it"*. Offline, no-fetch operation is therefore an existing accepted constraint of this codebase, not a new imposition by this plan.
...
**Not used — GitHub Pages.** The repository is private. Pages from a private repository is available on a paid plan and not on a Free one — so the claim depends on the account's plan, and **I cannot read the account's billing plan from this session and will not assert which applies.** That is the same standard this plan applies to Actions minutes in the paragraph above, and it has to hold in both directions. Either way this plan does not use Pages: it would rest on a subscription no session may assume or procure, and it is the owner's decision. The other routes to it are making this repository public, or creating a separate public repository holding only the built file. The first has disclosure consequences the owner should decide deliberately; the second is outside this repository's boundary and a session must not do it. Note also that a Pages site published *from* a private repository is itself public unless access control is configured — a disclosure consequence distinct from making the repository public. Recorded, not recommended, not acted on.

### 1.5 The viewing script

Alongside the build, Phase 1 delivers `docs/process/PHASE_1_HOW_TO_LOOK.md` — carrying the brief's `PHASE_1_` prefix so that it falls inside the permitted `docs/process/PHASE_1_*` pattern and needs no owner confirmation: one page per viewing point, in the owner's language, containing:
...
**And one thing it settles: the verdict the Phase 1 run record will carry.**

The commission closes its Phase 1 amendment (line 1974) with a recording rule that governs exactly this plan's situation:
...
That token is not a phrase. It is one of the three values of `verdict` in `schemas/run-record.schema.json` (`GO`, `NO_GO`, `BLOCKED_PENDING_REAL_GPU_REVIEW`), it is asserted by a live test (`packages/agent-contracts/test/run-record.test.ts`), and it is the standing verdict of Phase 0 (`PHASE_0_RUN_RECORD.md`; `docs/process/run-records/phase-0.run-record.json`). This plan's whole premise is that rule's trigger condition — software rendering cannot supply the visual evidence, so the owner's own hardware must — and a plan that defines the terminal deliverable must therefore say which way Phase 1 falls. It is stated here rather than left to the session that writes the record.

**The position this plan takes: the Owner Build is the mechanism that supplies the evidence software rendering cannot, so an owner PASS or PASS WITH DIRECTION recorded on the owner's own graphics hardware discharges the criterion.** The rule's condition is about whether sufficient evidence exists for the final visual judgement, not about where the artifact was built. An owner viewing on real hardware is that evidence, and it is a stronger basis than any container capture could be. This is a reading of a layer-1 rule and it is stated openly as one; if the owner reads it otherwise, the second branch below is what applies and Phase 1 ends exactly as Phase 0 did.

Concretely, and this is what T8.3 must implement:

- **`GO`** — if every owner viewing point required by the stage order has returned a recorded verdict of PASS or PASS WITH DIRECTION on the owner's own machine, and the other required checks are green. The run record must carry the viewing dates, the build SHA-256s and the statement that the visual judgement was made on real graphics hardware by the owner. It still carries the two OD-0005 deferrals as **not performed**: those are separate criteria and `GO` does not absorb them.
- **`BLOCKED_PENDING_REAL_GPU_REVIEW`** — if any required viewing point has not returned an owner verdict on real hardware: the owner cannot open the file, does not look, or the visual judgement would rest on container-rendered output. The record names which viewing point is missing. This is the same verdict Phase 0 carries and it is not a failure of the plan; it is the honest state.
- A **FAIL** at a viewing point is neither of those. It is an owner rejection of the look, and the work returns to the stage that produced it.

Two things this does not do. It does not turn an owner viewing verdict into a review verdict — Part 3's distinction stands, and the Keeper's verdict at S8 is separate, independent and unaffected. And the `goNoGo.verdict` in this document's appendix is the *plan's* verdict, not the run record's; they are different objects and must not be confused.

### 1.7 Consequence for an existing document

`docs/process/ART_DIRECTION_CHECKPOINT.md` describes a mechanism the owner will not use. Phase 1 should update it to point at the Owner Build; that is task T1.3 in Part 4.

**This plan cannot assert permission for it, and does not.** `PHASE_1_BRIEF.md` line 47 permits, under `docs/process/`, exactly one pattern: `docs/process/PHASE_1_*`. `ART_DIRECTION_CHECKPOINT.md` does not match it. So it is an owner path question, raised on the same terms as `docs/process/owner-builds/**` in Part 8: **recommend the owner confirm this one file by name as a permitted path for T1.3.** Until they do, T1.3's grant is `docs/process/PHASE_1_*`, the checkpoint record sits outside it, and no session may edit it — the document then stays as it is and the Owner Build mechanism is described only in `docs/process/PHASE_1_HOW_TO_LOOK.md`. **This session did not edit it in any case**, because it is not this session's job to edit a checkpoint record while planning against it.
...
| **S1 — The Owner Build** | Single-file build target, inliner, SHA footer, `PHASE_1_HOW_TO_LOOK.md`, one build of the *existing rejected spike* purely to prove the pipe | **V0 — "can you open it?"** Explicitly not an art judgement | 1 session | **One session, and the delivery mechanism itself.** If V0 passes, everything here is reused by every later stage. If it fails, the single-file target, the inliner and the viewing script go with it, and §1.4's alternative delivery route — outside the brief's permitted areas, and adding the Security Sentinel — becomes the question. |
...
| T1.2 | `PHASE_1_HOW_TO_LOOK.md` | T1.1 | `docs/process/PHASE_1_*` | knowledge-lint |
| T1.3 | Update `ART_DIRECTION_CHECKPOINT.md` to the Owner Build mechanism. **Blocked on the owner path question in §1.7**: that file does not match `docs/process/PHASE_1_*` | T1.1 | `docs/process/PHASE_1_*`; the checkpoint file only if the owner confirms it by name | knowledge-lint |
...
| T3.1 | Virgil geometry, materials, screen face | V1 | `apps/mission-control/src/characters/**`, `assets/**` + provenance row | `pnpm check`, character form gate (silhouette, joint occlusion, screen-face readability — Part 9) |
| T3.2 | Virgil idle and refusal poses | T3.1 | as T3.1 | `pnpm check`, character form gate |
...
| T8.1 | Prover verification against the T0.2 anchored required-check set. **If the owner declines T0.2, or it is not enacted before S8**, verification runs against this plan's appendix `requiredChecks` list, which is not owner-controlled; per `PHASE_1_BRIEF.md` line 15 that list and the Keeper's *verification incomplete* stop condition are then the only controls, and the KR-03 gap stands exactly as `ENFORCEMENT_BOUNDARIES.md` records it | V6 | `tests/**` within the authorised test boundary | all required checks |
| T8.2 | Art bible updated with refinements | V6 | `docs/art-direction/**` | knowledge-lint |
| T8.3 | Phase 1 run record. Names its `verdict` under commission line 1974 — `GO` only if every required owner viewing point returned PASS or PASS WITH DIRECTION on the owner's own hardware, otherwise `BLOCKED_PENDING_REAL_GPU_REVIEW` naming the missing viewing point (§1.6) — and states both deferred checks as **not performed** and why | T8.1 | `docs/process/PHASE_1_*` | knowledge-lint |
| T8.4 | Phase 2 brief proposal. **Owner path question** (Part 8): a `PHASE_2_*` filename is not covered by the brief's permitted pattern | T8.3 | `docs/process/PHASE_1_*` | knowledge-lint |
...
4. **Whether the code-authored character route works.** I believe it does: the approved designs are compact rounded volumes with distinctive equipment, which composes well from parametric geometry; and rigid chunky robots animate by transform hierarchy rather than skinned rigging, which removes the single hardest step. I have not built one — but **the claim that no one in this repository has is false and is corrected here**: `docs/process/BLENDER_FEASIBILITY_SPIKE.md` on `origin/claude/virgil-blender-feasibility` reports a generic robot part built, rigged, animated, rendered, exported and loaded through this application's own stack in **5.14 s**, with an edit → rebuild → render repair loop at **3.99 s**. Under `CLAUDE.md` that report is a builder's claim, not evidence: it has not been reproduced by this session and no independent reviewer has re-run it. Taken at that weight it still moves this item from *"nobody has tried"* to *"one authoring route is evidenced end to end and one is not"* (Part 9). It says nothing about charm; item 2 stands unchanged.
...
**Why it is available.** Commission 5.5 (`docs/product/VIRGIL_MASTER_COMMISSION.md` line 478) reads, in full:

> • At least three distinctive worker roles.

It names no roles, and the word *present* appears nowhere in 5.5. The parenthetical *"(Fabricator, Prover, Keeper, with Virgil, Cartographer and Architect present)"* is `PHASE_1_BRIEF.md` line 25 — a layer-4 process document, not the commission. An earlier version of this paragraph attributed that parenthetical to 5.5, which was wrong. The correction cuts in the proposal's favour: the top authority layer asks for *less* than the brief does, which is what makes B available at all. The brief's deliverable line says "modelled worker ensemble for the six visible roles per the role bible **or an owner-approved stylisation direction**". That clause is the room. And OD-0002 approved exactly four character sheets: Virgil, Fabricator, Prover, Keeper. There are **no approved sheets for the Cartographer or the Architect**, so modelling them at all means a session inventing two characters the owner has never seen — which is a worse problem than not modelling them.

**What it costs, and at which authority layer.** The commission's own Phase 1 vertical-slice amendment (lines 1922–1949, inside the layer-1 document that `CLAUDE.md` governs *"including its amendments register"*) requires as beats 2 and 3 of the mandatory story: *"The Cartographer visibly bounding it"* and *"The Architect creating the plan without building."* Proposal B represents both as station-bound presences. **So B is not only a reduction against a layer-4 deliverables list: it asks the owner to interpret a layer-1 requirement — whether a lit station performing the bounding ritual, with its instruments and its working ritual, satisfies "visibly bounding".** That may well be a sound reading; the amendment requires the act to be visible, not that a fully modelled character perform it. But it is the owner's reading to make, made knowingly, and this plan should not have let them believe they were trading away only a process document.
...
- It is a genuine reduction against the brief's deliverables list **and an interpretation of beats 2 and 3 of the commission's layer-1 Phase 1 amendment**. Both must be recorded as such, not slipped through.
...
**Recommended default at the time of writing: (a).** It restores a green base in one line each, and it does not require deciding a governance question under time pressure.

**That recommendation is overtaken, and this paragraph corrects it.** Since this plan was written, `main` has advanced `118aa44` → `dd8ddb1` → `b0ee347` → `e3464a2`, three commits all titled *"Update authority.json"*, whose net effect is a new `boundaryProtection` object in `constitution/authority.json` splitting `sessionDenied` (`constitution/`, the commission, `knowledge/raw/`, `schemas/gate-*`) from `ownerInstructedOnly` (`docs/decisions/OD-*`). **That is the substance of option (b), written into the constitution by the owner's own hand** — which is exactly where it belongs, since the classification of what is protected must live where no session can reach it. Option (a) now runs against the owner's expressed direction and is no longer recommended.

**The finding itself stands.** The failing assertion reads `authority.protectedBoundaries` and `settings.permissions.deny`; neither moved, so it still fails. The corollary stands too: OD-0002 and OD-0005 each cite the removed deny rule as their reason for remaining at `proposed/`, and that control does not exist.

**What remains, and who does it.** The test-side work that option (b) described — making the check read `boundaryProtection` rather than `protectedBoundaries` — is in flight separately on branch `claude/virgil-file-decisions` (pull request #5), and is not this plan's to schedule or to claim. That pull request reports that what now stops `pnpm check` first is a formatting deviation inside the owner's hand-edit — `ownerInstructedOnly` indented two spaces where four are required, which I confirmed is still present at `e3464a2` — and that no session may repair it, because `constitution/` is protected from every session. Phase 1 must still start from a green base, so **P0 stays blocking**; what it needs from the owner is now the whitespace correction and a decision on that separate pull request, not a choice between (a) and (b).

**One contradiction, reported and not resolved**, per `CLAUDE.md`: `constitution/authority.json` now holds two lists that disagree about `docs/decisions/OD-*` — `protectedBoundaries` includes it, `boundaryProtection.sessionDenied` excludes it. That is the owner's layer and no session resolves it.

**This session did not touch `.claude/settings.json`, `constitution/` or `packages/`, and could not: all three are outside its boundary.**
...
docs/process/PHASE_1_HOW_TO_LOOK.md
docs/process/owner-builds/**         (new: the Owner Build artifacts)
assets/**                            (only with a provenance row written first)
tests/**
```

**Path questions for the owner**, all raised rather than assumed. `PHASE_1_BRIEF.md` line 47 permits, under `docs/process/`, exactly `docs/process/PHASE_1_*`, and nothing else in that directory:
...
- `docs/process/ART_DIRECTION_CHECKPOINT.md` (task T1.3) does not match the pattern and cannot be renamed — it is an existing Phase 0 record. There is no prefix trick available, so this one is a real question: **recommend the owner confirm that single file by name as a permitted path for T1.3, or that T1.3 is left unexecuted and the correction lives only in `docs/process/PHASE_1_HOW_TO_LOOK.md`.** No session may edit the checkpoint record without that confirmation (§1.7).
- The **Phase 2 brief proposal** (task T8.4) has the same shape: a `PHASE_2_*` filename is not covered by `docs/process/PHASE_1_*`. Recommend the owner confirm the path, or that the proposal is filed under a `PHASE_1_`-prefixed name (for example `docs/process/PHASE_1_PHASE_2_BRIEF_PROPOSAL.md`) so that no confirmation is needed. **Default: use the `PHASE_1_` prefix.**
...
**Is it achievable by sessions alone? My answer: probably yes, by either of two routes, with one specific residual risk.**

- **Route 1 — parametric geometry authored in code.** Compose the approved silhouettes from geometry in `three.js`, with the screen face as a shader-driven emissive panel, and animate by transform hierarchy rather than skinned rigging. The approved designs make this unusually tractable: compact rounded volumes with distinctive bolt-on equipment, and rigid chunky bodies that do not need a skeleton. Everything is authored in this repository from the owner's own sheets, so the licence question disappears entirely.
- **Route 2 — authored in Blender, exported as glTF, loaded through the application's own stack.** `docs/process/BLENDER_FEASIBILITY_SPIKE.md` on `origin/claude/virgil-blender-feasibility` reports `bpy` installing headless in about 22 s and 925 MiB with no GPU, and a build → rig → animate → render → export → `GLTFLoader.parse` chain end to end at **5.14 s**, with a repair loop at **3.99 s**. **That report is a builder's claim and not evidence** (`CLAUDE.md`): this session did not reproduce it, and no independent reviewer has re-run its scripts. Two of its findings bear on either route. *Construction:* it measures bone parenting preserving every rigid shell to **≤ 2.1e-07 m** under pose, while armature deform with automatic weights bends the torso by **1.09e-01 m** and the flat screen face by **1.5e-02 m** — for hard ceramic panels bone parenting is the correct construction and automatic weights are simply wrong. *Dependency:* `bpy` 5.0.1 declares `Requires-Python: ==3.11.*`, so this route rests on the container image staying on Python 3.11, and `bpy` would be a new tool dependency needing an ADR and a licence row by §1.3's route-2 logic. **This plan does not choose between the two routes.** The choice belongs to the session that reaches S3, on the evidence as it stands then; neither route changes a stage, a task, a viewing point or an estimate.
- **What a container can decide before the owner ever looks.** The same spike claims — again as a builder's claim — that *form* is measurable without a GPU: silhouette is renderer-independent (alpha IoU 0.999738 between Cycles and EEVEE, 14 disagreeing pixels of 230,400, all on antialiased edges); joint occlusion is exact by pixel count across a 24-angle turntable (0 exposed pixels assembled, against 1,063 on a negative control that proves the test can detect exposure); and screen-face readability against on-screen size is a locatable curve (contrast 0.715 at 480 px falling to 0.358 at 48 px, collapsing between 96 px and 64 px). If that holds, a character with a measurably broken rig, an exposed joint or a face that stops reading at target size should never reach an owner viewing point. Part 4 therefore carries a **character form gate** — silhouette stability, joint occlusion, screen-face readability — as a required check on T3.1 and T3.2, before V2. It judges form only: appearance still cannot be judged in a container (§1.2, §1.6), and charm still cannot be judged by anything but the owner.
...
```json
{
  ...
  "repositoryFindings": [
    ...
    "P0 disposition, corrected after main moved to e3464a2: the owner has written the substance of option (b) into constitution/authority.json as a boundaryProtection object splitting sessionDenied from ownerInstructedOnly (docs/decisions/OD-*), so option (a) is no longer recommended. The finding still reproduces because the failing assertion reads protectedBoundaries and settings.permissions.deny, neither of which moved. The test-side work is in flight separately on pull request #5, which reports that pnpm check now stops first on a formatting deviation inside the owner's hand-edit of constitution/authority.json that no session may repair. Reported and not resolved: protectedBoundaries and boundaryProtection.sessionDenied now disagree about docs/decisions/OD-*.",
    "docs/process/BLENDER_FEASIBILITY_SPIKE.md on origin/claude/virgil-blender-feasibility reports a Blender-authored character route measured end to end at 5.14 s with a 3.99 s repair loop, and reports that silhouette, joint occlusion and screen-face readability are measurable in a container without a GPU. Treated as a builder's claim, not evidence: not reproduced by this session and not independently reviewed. It falsifies the plan's earlier statement that nobody in this repository has built one, and it is the basis of the character form gate on T3.1 and T3.2.",
    "The permitted docs/process/ pattern in PHASE_1_BRIEF.md line 47 is exactly docs/process/PHASE_1_*. docs/process/ART_DIRECTION_CHECKPOINT.md (task T1.3) and a PHASE_2_* brief proposal (task T8.4) are outside it and are raised as owner path questions rather than assumed.",
  ...
  "taskGraph": [
    ...
    { "taskId": "T1.2", "title": "PHASE_1_HOW_TO_LOOK.md viewing script", "dependsOn": ["T1.1"], "permittedPaths": ["docs/process/PHASE_1_*"], "requiredChecks": ["knowledge-lint"] },
    { "taskId": "T1.3", "title": "Update ART_DIRECTION_CHECKPOINT.md to the Owner Build mechanism (blocked on an owner path question: that file does not match docs/process/PHASE_1_*)", "dependsOn": ["T1.1"], "permittedPaths": ["docs/process/PHASE_1_*"], "requiredChecks": ["knowledge-lint"] },
    { "taskId": "V0", "title": "Owner viewing point: can the file be opened", "dependsOn": ["T1.1", "T1.2", "T1.3"], "permittedPaths": [], "requiredChecks": ["owner-decision"] },
    ...
    { "taskId": "T2.2", "title": "Foundry hero composition: sky, nebula, station, docks, gateway, lighting, materials, bloom", "dependsOn": ["T2.1"], "permittedPaths": ["apps/mission-control/src/world/**", "apps/mission-control/src/scenes/**"], "requiredChecks": ["pnpm-check"] },
    { "taskId": "T2.3", "title": "Mind hero composition", "dependsOn": ["T2.1"], "permittedPaths": ["apps/mission-control/src/world/**", "apps/mission-control/src/scenes/**"], "requiredChecks": ["pnpm-check"] },
    ...
    { "taskId": "V1", "title": "Owner viewing point: the art bar", "dependsOn": ["T2.2", "T2.3", "T2.4"], "permittedPaths": [], "requiredChecks": ["owner-decision"] },
    { "taskId": "T3.1", "title": "Virgil geometry, materials and screen face from the approved sheets", "dependsOn": ["V1"], "permittedPaths": ["apps/mission-control/src/characters/**", "assets/**"], "requiredChecks": ["pnpm-check", "character-form"] },
    { "taskId": "T3.2", "title": "Virgil idle and refusal poses", "dependsOn": ["T3.1"], "permittedPaths": ["apps/mission-control/src/characters/**", "assets/**"], "requiredChecks": ["pnpm-check", "character-form"] },
    { "taskId": "V2", "title": "Owner viewing point: the character bar", "dependsOn": ["T3.1", "T3.2"], "permittedPaths": [], "requiredChecks": ["owner-decision"] },
    ...
    { "taskId": "V3", "title": "Owner viewing point: motion and truth", "dependsOn": ["T4.1", "T4.2", "T4.3"], "permittedPaths": [], "requiredChecks": ["owner-decision"] },
    { "taskId": "T5.1", "title": "Fabricator, Prover, Keeper, Cartographer, Architect (split across sessions; subject to Proposal B)", "dependsOn": ["V2", "V3"], "permittedPaths": ["apps/mission-control/src/characters/**", "assets/**"], "requiredChecks": ["pnpm-check"] },
    ...
    { "taskId": "V4", "title": "Owner viewing point: the Foundry slice", "dependsOn": ["T5.1", "T5.2", "T5.4", "T5.7"], "permittedPaths": [], "requiredChecks": ["owner-decision"] },
    ...
    { "taskId": "V5", "title": "Owner viewing point: the Mind", "dependsOn": ["T6.1", "T6.2", "T6.3"], "permittedPaths": [], "requiredChecks": ["owner-decision"] },
    ...
    { "taskId": "V6", "title": "Owner viewing point: the modes", "dependsOn": ["T7.1", "T7.2", "T7.3"], "permittedPaths": [], "requiredChecks": ["owner-decision"] },
    { "taskId": "T8.1", "title": "Prover verification against the T0.2 anchored required-check set; if T0.2 is declined or not enacted, against this plan's requiredChecks list, which is not owner-controlled, leaving the KR-03 gap exactly as recorded", "dependsOn": ["V6"], "permittedPaths": ["tests/** (within the authorised test boundary)"], "requiredChecks": ["pnpm-check", "playwright"] },
    ...
    { "taskId": "T8.3", "title": "Phase 1 run record: names its verdict under commission line 1974 - GO only if every required owner viewing point returned PASS or PASS WITH DIRECTION on the owner's own graphics hardware, otherwise BLOCKED_PENDING_REAL_GPU_REVIEW naming the missing viewing point - and states both deferred checks as not performed and why", "dependsOn": ["T8.1"], "permittedPaths": ["docs/process/PHASE_1_*"], "requiredChecks": ["knowledge-lint"] },
    { "taskId": "T8.4", "title": "Phase 2 brief proposal (owner path question: a PHASE_2_* filename is not covered by the brief's permitted pattern)", "dependsOn": ["T8.3"], "permittedPaths": ["docs/process/PHASE_1_*"], "requiredChecks": ["knowledge-lint"] },
  ...
  "requiredChecks": [
    ...
    { "checkId": "character-form", "name": "Character form gate: silhouette stability across renderers, joint occlusion by pixel count across a turntable, and screen-face readability contrast at target on-screen size", "commandClass": "form-gate" },
