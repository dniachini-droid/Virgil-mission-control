...
| BR-01 | moderate | `docs/process/KNOWLEDGE_LESSONS_BRIEF.md` criterion 7; `apps/mission-control/e2e/mutation-manifest.ts` | Read criterion 7 against the brief's permitted paths; then read the manifest's `cwd: app` against `apps/mission-control/vitest.config.ts`, which includes `test/**` only | The brief's own permitted-paths contract; `REVIEW_POLICY.md` on a contract a candidate can satisfy |
