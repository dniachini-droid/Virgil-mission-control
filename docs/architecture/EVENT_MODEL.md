...
The fifteen states and the transition table are in `constitution/authority.json` and explained in `constitution/STATE_LANGUAGE.md`. The reducer applies them in three stages, described in full in `ENFORCEMENT_BOUNDARIES.md`: event validation (`packages/domain/src/validation.ts`), then the transition table with its guards (`guards.ts`), then on-transition effects. A rejected event is recorded with a kind (`order`, `contract`, `authority`, `consistency`, `transition`) and applies no effect; every event is parsed against the `DomainEvent` contract before anything else, so an unknown type, actor kind or authority tag never reaches the read model.
...
| `actor_is_owner` | owner actor citing a recorded, unconsumed owner decision of kind `merge` whose `appliesToSha` is the current candidate SHA (the merge-gate checks for `merged_by_owner` run in event validation) |
