...
**Classification**, in the status vocabulary of `docs/architecture/ENFORCEMENT_BOUNDARIES.md`, updated on 2026-09-07 when part of it stopped being true.

The single-channel condition is **design-level only**. No code determines which channel an instruction arrived on, and nothing here can.

The verbatim-quote requirement now splits in two, and the halves are not equal:

- The **presence** of a quotation is **implemented now** and **validated by tests**. `.claude/hooks/od-decision-record-guard.mjs`, wired as a `PreToolUse` hook on `Write` and `Edit`, refuses a write to `docs/decisions/OD-*` whose resulting file lacks either a blockquoted verbatim quotation or a `YYYY-MM-DD` date; `packages/agent-contracts/test/od-decision-record-guard.test.ts` fails if either check is removed. A record filed without the owner's words can no longer be written through those two tools, where before nothing stopped it.
- The **authenticity** of a quotation is **design-level only**, and this is the half that carries the risk. Nothing checks that the owner said the quoted words, or said anything. The guard holds no copy of the owner's instruction, and neither does this repository. A session that invents a quotation and a date satisfies it completely and the guard reports nothing. Nor is the guard unavoidable: it runs on the `Write` and `Edit` tools, so a write performed through `Bash` bypasses it entirely.

The `owner_decision` event payload (`packages/agent-contracts/src/events.ts`) carries `decisionId`, `kind`, `resumesTo` and `appliesToSha`, and the `OwnerDecisionRecord` contract (`packages/agent-contracts/src/operational.ts`) carries `question`, `decision` and `consequences`; neither has a field for the owner's words, so the reducer still has nothing to check. `.claude/settings.json` denies paths, not content; the hook is what looks at content. **The residual risk stated above is unchanged by the hook and is not reduced by it.** The single-channel condition, and the truthfulness of any quotation, remain procedures a session is asked to follow, not controls that stop a session that does not. The matching rows are recorded in `ENFORCEMENT_BOUNDARIES.md` so the two documents agree; no other status is claimed for them, because no other status has been verified.
...
- The verbatim-quote requirement was procedural and unenforced when this decision was recorded. Since 2026-09-07 a `PreToolUse` hook refuses a `Write` or `Edit` to `docs/decisions/OD-*` whose result carries no quotation and no date, so a session that simply ignores the requirement is now stopped through those two tools. Nothing stops or flags a session that files a quote the owner never spoke, and nothing stops a session that writes the file through `Bash` instead. Recording that in `ENFORCEMENT_BOUNDARIES.md` records the gap that remains; the hook narrows it and does not close it.
