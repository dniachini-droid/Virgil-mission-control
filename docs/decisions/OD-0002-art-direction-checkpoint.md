...
Why it stood at `proposed/` for a time, and why it no longer does: when the consolidation session transcribed it, `docs/decisions/README.md` said a proposal had no authority until the owner moved it, `CLAUDE.md` said only the owner changes authority layer 1, `constitution/authority.json` listed `docs/decisions/OD-*` as a protected boundary, and `.claude/settings.json` denied `Edit` and `Write` under that path. That session was instructed not to circumvent the policy and did not. The owner then removed those two deny lines themselves, in commit `9627bae`, and decided in `OD-0006-recording-owner-decisions.md` that their instruction in the owner console is sufficient authority for a session to file a decision. This record was filed at the accepted path under that mechanism on 2026-09-07, and the paragraph that used to stand here — which said this file was at `proposed/` — was corrected on the same date because it had become false. Nothing else in this record has been altered.
