...
|---|---|---|
| Domain state-machine tests | `packages/domain/test` | 30 tests: table integrity, five fixture runs, distinct states, authority violations, out-of-order rejection, determinism |
...
| Provenance-graph derivation and replay | `packages/knowledge-graph/test/derive.test.ts` | Reproducibility, immutability, tether resolution, rank capping |
...
Totals in Phase 0: 130 unit tests across six packages, one capture script, one Mind Scan CLI. `pnpm check` runs lint, typecheck and tests.
