...
import { type CandidateState, type LineageState, newLineage, type RunState } from './state.js';
import { candidateTransitions, lineageEventTypes, softLineageEvents } from './transitions.js';
  ...
  switch (event.type) {
    ...
    case 'agent_started': {
      ...
      if (p.roleId === 'fabricator') {
        ...
        run.lineage.builderSessions.push(sessionId);
      }
      break;
    }
    case 'agent_waiting': {
    ...
    case 'check_started':
      if (run.lineage)
        run.lineage.checks[str(p.checkId)] = {
          checkId: str(p.checkId),
          name: str(p.name),
    ...
    case 'verification_completed':
      if (
        run.lineage &&
        p.allRequiredCompleted === true &&
        p.anyRequiredFailed === false &&
...
export function applyEvent(input: RunState, event: DomainEvent): ApplyResult {
  ...
  if (event.seq <= run.lastSeq) {
    run.invalidTransitions.push({
      ...
      reason: `out-of-order seq ${event.seq} after ${run.lastSeq}`,
    });
    return { run, accepted: false, transitioned: false, reason: 'out_of_order' };
  }
  ...
  if (!lineage) {
    ...
    run.invalidTransitions.push({
      ...
      type: event.type,
      state: null,
      reason: 'no candidate lineage exists yet',
    });
    return { run, accepted: false, transitioned: false, reason: 'no_lineage' };
  }
  ...
  if (match.to === '$resumesTo') {
    ...
    if (!resumesTo) {
      run.invalidTransitions.push({
        ...
        type: event.type,
        state: lineage.state,
        reason: 'owner_decision without resumesTo and no resume state',
      });
      return { run, accepted: false, transitioned: false, reason: 'no_resume_state' };
    }
    to = resumesTo;
  } else {
    to = match.to;
  ...
  lineage.state = to;
  applyOnTransition(run, lineage, event, to);
  if (from !== to)
