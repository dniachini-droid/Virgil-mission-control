...
function liveProverDoc(state: DemoState, checks: NonNullable<DemoState['checks']>): WindowDoc {
        ...
        : running > 0
          ...
          : noResult > 0
            ...
            : {
                ...
                meaning: `Every check ${checks.source} reported against this version passed. That is the Prover's evidence. It is not a verdict on the work.`,
  ...
  const sections: Section[] = [
    {
      id: 'checks',
      title: total === 0 ? 'The checks that reported' : `The ${checkWord(total)} that reported`,
      summary:
        rows.length === 0
          ? `Nothing from ${checks.source}`
          : `${passed} passed · ${failed} failed · ${running} still running · ${skipped} skipped`,
      open: true,
      blocks: [
        ...(rows.length > 0
          ...
          ? ([
              {
                ...
                text: `${noResultLine}. They are counted here and left out of the list, because a list of results is a claim about every line in it.`,
              },
            ] as Block[])
          : []),
        ...(rows.length === 0 && noResult === 0
          ? ([
              {
                kind: 'note',
                text: `${checks.source} reported no checks at all against this version.`,
              },
            ] as Block[])
          : []),
      ],
...
function proverDoc(state: DemoState): WindowDoc {
  // Real checks answer the question this window asks, so they answer it. The
  // recording below is what a scripted run has, not a fallback the live path
  // may quietly borrow from.
